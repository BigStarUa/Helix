...
public class DistributorsList extends AbstractJPanel implements DistributorsListListener{
	
	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	private static DistributorsList distribList;
	...
	private DistributorManager pm;
	private DistributorsListTableModel model;
	private JPanel panel;
	private JLabel lblFilter;

	private DistributorsList(){
		setLayout(new BorderLayout());
		window = SwingUtilities.getWindowAncestor(DistributorsList.this);
		
		// Getting list of product from database
		pm = new DistributorManagerImpl();
		List<Distributor> list = pm.loadAllProductsOrderedBy("name");
		
		//list = new ArrayList<>();
				
		//Creating table model from list of products
		model = new DistributorsListTableModel(list);
			...
			public void mouseClicked(MouseEvent e) {
				...
				if(row > -1 && e.getButton() == MouseEvent.BUTTON1){
					if (e.getClickCount() == 2){
				        ...
				        Distributor distributor = (Distributor) table.getModel().getValueAt(row, -1);
				        JDialog pf = new DistributorForm(window, "Distributor form", Dialog.ModalityType.DOCUMENT_MODAL, distributor, DistributorsList.this);
						pf.setVisible(true);
					}
				}
			}
...
//					toolBar.revalidate();
	...
	public static synchronized DistributorsList getInstance(){
		if(distribList == null){
			distribList = new DistributorsList();
		}
		return distribList;
	}
		...
		delButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if(row > -1){
					...
					{
						DistributorsListTableModel model = (DistributorsListTableModel)table.getModel();
					    
//					    List<Distributor> list = pm.loadDependentProductsList(((Distributor)model.getValueAt(row, -1)).getId());
//					    if(list != null && list.size() > 0){
//					    	StringBuilder sb = new StringBuilder();
//					    	sb.append("You can't delete this Distributor. It is used\n");
...
//					    }else{
		...
		filterText.addKeyListener(new KeyAdapter()
		{
			public void keyReleased(KeyEvent ke)
			{
		        ...
		        if(table.getSelectedRow() == -1){
		        	editButton.setEnabled(false);
		        	DistributorsList.this.toolBar.repaint();
		        	DistributorsList.this.toolBar.revalidate();
		        }
	...
	public void saveDistributor(Distributor distributor) {
		...
		if(distributor.getId() > 0){
			pm.updateProduct(distributor);
			System.out.println("Product updated");
		}else{
			pm.saveNewProduct(distributor);
			model.addDistributor(distributor);
